...
MainWindow::MainWindow()
{
    ...
    button_phong_import = new QPushButton("导入Phong计算数据");
    button_phong_import->setEnabled(false);

    phong_texture = new QImage(360,90,QImage::Format_RGB32);
...
float cosineDistribution(float value, float order){
    return pow(cos(value),order);
}
...
void MainWindow::onButtonCalcPhongMapClicked(){
    ...
    for(int out_theta = 89; out_theta>=0; --out_theta){
        for(int out_phi = 0; out_phi < 360; ++ out_phi){
            ...
            half.normalize();
            float brightness = QVector3D::dotProduct(half,QVector3D(0,1,0))*cosineDistribution(QVector3D::dotProduct(half,QVector3D(0,1,0)),spinbox_phong_order->value());
